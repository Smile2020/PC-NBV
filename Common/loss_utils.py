...
def discriminator_new_loss(D, input_real, input_fake, inputs, npts, batch_size, Ra=False, gan_type='lsgan'):
    ...
    for input_single in tf.split(inputs, npts, axis=1):
        ...
        dists_forward, _, dists_backward, _ = tf_nndistance.nn_distance(single_fake, input_single)
        ...
        dists_forward, _, dists_backward, _ = tf_nndistance.nn_distance(single_real, input_single)
        dist_index = tf.where(dists_forward > 0.005)
